...
const PostDetails = () => {
  ...
  const { publishedAt, title, views, image, description, author } = data?.data[0]
  return (
    <CustomContainer>
        ...
        {data?.data && <>
          <Box display={'flex'} marginTop={'40%'} alignItems={'start'} flexDirection={'column'} justifyContent={'center'} gap={3}>
            <Box display={'flex'} flexDirection={'column'} alignItems={'center'} gap={3}>
              <Image
                src={data?.data[0].author?.image || '/images/user.png'}
                boxSize="120px"
                borderRadius="full"
                fit="cover"
                alt="user-image"
          ...
          <Box>
            <Box width={'full'}>
              <Box>
                <Image className='post-details-image' src={`${image.url}`} alt="post-image" />
              </Box>
              <Box marginTop={'10px'}>
                ...
                <Box display={'flex'} gap={5} marginTop={'10px'} color={'gray.400'}>
                  <Text fontSize={'12px'} className='date'>{moment(publishedAt).format('DD.MM.YYYY')}</Text>
